...
/* ============================================================
   ...
   ============================================================ */
class Somo {
  final String code;    // mf. "MATH" — uppercase
  final String name;    // mf. "Mathematics"
  ...
  Somo copyWith({String? code, String? name, String? level, bool? active}) =>
      Somo(
        code: code ?? this.code,
        name: name ?? this.name,
...
/* ============================================================
   1. ANGALIA
   ============================================================ */
Future<void> showSomoViewDialog(
          ...
          children: [
            ...
            _Table(c: c, children: [
              _ViewRow(
                c: c,
                label: 'MSIMBO',
                value: Text(somo.code,
                    style: TextStyle(
                        color: c.text, fontSize: 13, fontFamily: 'monospace')),
              ),
              _ViewRow(c: c, label: 'JINA', value: _val(c, somo.name)),
              _ViewRow(
                c: c,
                label: 'KIWANGO',
                value: somo.level == 'Primary'
                    ? _Pill(label: somo.level, fg: c.blue, bg: c.blueBg)
                    : _Pill(label: somo.level, fg: c.amber, bg: c.amberBg),
              ),
              _ViewRow(
                c: c,
                label: 'HALI',
...
/* ============================================================
   2. HARIRI (au ONGEZA ukipitisha somo = null)
   ============================================================ */
Future<Somo?> showSomoEditDialog(BuildContext context, {Somo? somo}) {
  return showDialog<Somo>(
    context: context,
    builder: (_) => _EditDialog(original: somo),
  );
}
...
class _EditDialogState extends State<_EditDialog> {
  ...
  int get changes =>
      ...
      (level != (o.level.isEmpty ? 'Primary' : o.level) ? 1 : 0) +
      (active != o.active ? 1 : 0);
              ...
              children: [
                Expanded(
                  child: Text(
                    ...
                    style:
                        TextStyle(color: n == 0 ? c.muted : c.blue, fontSize: 12),
                  ),
                ),
                _SmallBtn(
                    label: 'Ghairi',
...
/// Msimbo: herufi kubwa, nafasi -> "_"
class _UpperFormatter extends TextInputFormatter {
  @override
  TextEditingValue formatEditUpdate(
      TextEditingValue oldValue, TextEditingValue newValue) {
    final t =
        newValue.text.toUpperCase().replaceAll(RegExp(r'\s+'), '_');
...
/* ============================================================
   3. FUTA
   ============================================================ */
Future<bool> showSomoDeleteDialog(BuildContext context, Somo somo) async {
